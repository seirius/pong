...
public class EvolutiveIA {
		...
		for (int o = 0; o < outputs; o++) {
			...
			for (int o1 = 0; o1 < outputs; o1++)
				weights.get(weights.size() - 1).add(
						(new Random().nextGaussian() - 0.5) / 0.01);
		}
	}
		...
		for (int o = 0; o < outputs; o++) {
			...
			for (int i = 0; i < inputs; i++) {
				output.set(output.size() - 1, output.get(output.size() - 1)
						+ weights.get(o).get(i) * input.get(i));
			}
			output.set(output.size() - 1, sigmoid(output.get(output.size() - 1)
					+ weights.get(o).get(output.size() - 1)));
		}
		return output;
	}
	...
	public void update(int getPoints, int objPoints) {
		...
		for (int o = 0; o < outputs; o++) {
			...
			for (int o1 = 0; o1 < outputs; o1++)
				newWeights.get(newWeights.size() - 1).add(
						weights.get(o).get(o1)
								+ (new Random().nextGaussian() - 0.5) * 2
								/ (objPoints / getPoints));
		}
	}
